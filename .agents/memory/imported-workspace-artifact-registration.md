...
## Side effect: .migration-backup duplicates

`runPostMergeSetup()` also discovers artifacts inside `.migration-backup/` and registers them as duplicates. Stop those extra workflows immediately after with `stopWorkflow()`. The `.migration-backup/` directory is a Replit import artifact — it can be left on disk but its workflows should be stopped.
