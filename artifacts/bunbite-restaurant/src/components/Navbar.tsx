...
import { ShoppingCart, CalendarClock, LogOut, User } from 'lucide-react';
...
export default function Navbar() {
  ...
  const AuthActions = ({ mobile = false }: { mobile?: boolean }) => (
    <div className={`flex items-center ${mobile ? 'gap-6 justify-center' : 'gap-3'}`}>
      ...
      >
        <ShoppingCart size={mobile ? 22 : 18} />
        {/* Badge placeholder */}
      ...
      {user && (
        <div ref={mobile ? undefined : profileRef} className="relative">
          <button
            onClick={() => setProfileOpen((o) => !o)}
            aria-label="Profile menu"
            aria-expanded={profileOpen}
            className={`flex items-center justify-center rounded-full overflow-hidden font-bold tracking-wide transition-all ring-2 ring-secondary/60 hover:ring-secondary
              ${mobile ? 'w-14 h-14 text-base' : 'w-10 h-10 text-xs'}`}
