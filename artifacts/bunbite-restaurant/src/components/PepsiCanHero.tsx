...
export default function PepsiCanHero() {
  ...
  /* Translation & scale for drag/float */
  const x     = useMotionValue(0);
  const y     = useMotionValue(0);
  const scale = useMotionValue(1);

  /* 3-D tilt via rotateX / rotateY springs */
  ...
  const handleMouseEnter = useCallback(() => {
    if (isDragging.current || isTouch) return;
    animate(y,      -10, HOVER_SPRING);   // float up
    animate(scale,  1.07, HOVER_SPRING);
    animate(rotateZ, 4,   HOVER_SPRING);  // slight clockwise tilt
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      /* Slight tilt while dragging based on velocity direction */
  ...
  return (
    ...
    >
      {/* Glow */}
      <div
        className="absolute inset-x-[20%] bottom-0 h-[30%] rounded-full blur-2xl opacity-30 pointer-events-none"
